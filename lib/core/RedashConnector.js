...
exports.Connector = class RedashConnector {
  ...
  provision(context) {
    ...
    return this.accountManager.provisionUser(mContext)
      .then(util.removeCredentialFromUser)
      .then(user => ({ user }));
  };
  ...
  show(context) {
    ...
    return this.accountManager.showUsers(mContext)
      ...
      .then(user => {
        ...
        user = util.removeCredentialFromUser(user);
        return { user };
      });
  };
    ...
    return this.accountManager.showUsers(mContext)
      .then(data => new BatchIterator(this, {
        results: data.results
          .map(util.removeCredentialFromUser)
          .map(user => ({ user })),
        page: data.page,
  ...
  listAvailableAccessContext(context) {
    let mContext = { ...context };
    return this.accountManager.listAvailableGroups(mContext);  
  }
};
