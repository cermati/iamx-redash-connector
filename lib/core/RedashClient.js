...
/**
 * @typedef {import('./Schema').RedashUser} RedashUser
 */
...
exports.RedashClient = class RedashClient {
   ...
   * - Payload: JSON {"email": "test@cermati.com", "name": "Test user", "group_id": 3}
   *
   * @param {string} email
   * @param {string} name
   ...
   * @returns {Promise}
   */
  createUser (email, name, group_id = 2) {
    this._mustLogin();
   ...
   *
   * Example Request: DELETE /api/users/330/disable
   *
   * @param {string} email
   * @returns {Promise}
   */
   ...
   * @returns {Promise}
   */
  addUserGroups (userId, groupId) {
    this._mustLogin();

    let uri = `${this.config.baseUri}/api/groups/${groupId}/members`
    let payload = {
      "user_id": userId
    };
